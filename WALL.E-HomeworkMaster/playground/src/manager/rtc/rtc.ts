...
import { createImageVideoTrack } from "./imageTrack";
...
export class RtcManager extends AGEventEmitter<RtcEvents> {
  ...
  private imageStream?: MediaStream;
  private stopImageRenderLoop?: () => void;
  ...
  async setImageSource(imageUrl: string) {
    this.lastImageUrl = imageUrl;
    try {
      if (this.imageStream) {
        this.imageStream.getTracks().forEach((track) => track.stop());
      }
      if (this.stopImageRenderLoop) {
        this.stopImageRenderLoop();
        this.stopImageRenderLoop = undefined;
      }
      const { track, canvas, stream, stopRendering } = await createImageVideoTrack(
        AgoraRTC,
        imageUrl,
        {
          existingTrack: this.localTracks.imageTrack,
        },
      );
      this.localTracks.imageTrack = track;
      this.imageCanvas = canvas;
      this.imageStream = stream;
      this.stopImageRenderLoop = stopRendering;
      this.emit("localTracksChanged", this.localTracks);
    } catch (error) {
      console.error("Failed to create image video track", error);
      throw error;
    }
  }

  clearImageSource() {
    if (this.localTracks.imageTrack) {
      try {
        this.localTracks.imageTrack.stop();
        this.localTracks.imageTrack.close();
      } catch (err) {
        console.warn("Failed to stop image track", err);
      }
    }
    if (this.stopImageRenderLoop) {
      this.stopImageRenderLoop();
    ...
    this.localTracks.imageTrack = undefined;
  ...
  async switchVideoSource(type: VideoSourceType) {
    if (!this._joined) {
      console.warn("RTC client not joined; delaying video source switch");
    }
    if (type === VideoSourceType.SCREEN) {
      ...
      if (this.localTracks.screenTrack) {
        ...
        if (this.localTracks.imageTrack) {
          tracksToUnpublish.push(this.localTracks.imageTrack);
        }
        if (this._joined && tracksToUnpublish.length) {
    ...
    } else if (type === VideoSourceType.IMAGE) {
      if (!this.localTracks.imageTrack) {
        if (!this.lastImageUrl) {
          throw new Error("No uploaded image available for video source switch.");
        }
        await this.setImageSource(this.lastImageUrl);
      }
      if (this.localTracks.imageTrack) {
        const tracksToUnpublish: any[] = [];
        if (this.localTracks.videoTrack) {
        ...
        this.localTracks.screenTrack = undefined;
        if (this._joined) {
          await this.client.publish(this.localTracks.imageTrack);
        }
        this.emit("localTracksChanged", this.localTracks);
      }
    }
