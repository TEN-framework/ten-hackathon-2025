...
import { MonitorIcon, MonitorXIcon, ImageIcon } from "lucide-react"
...
export const ImageIconByStatus = (
  props: React.SVGProps<SVGSVGElement> & { active?: boolean; color?: string },
) => {
  const { active, color, ...rest } = props
  if (active) {
    return <ImageIcon color={color || "#3D53F5"} {...rest} />
  }
  return <ImageIcon color={color || "#667085"} {...rest} />
}
...
export default function VideoBlock(props: {
  ...
  imageTrack?: ILocalVideoTrack,
}) {
  const { videoSourceType, cameraTrack, screenTrack, imageTrack, onVideoSourceChange } = props
  const [videoMute, setVideoMute] = React.useState(false)

  React.useEffect(() => {
    cameraTrack?.setMuted(videoMute)
    screenTrack?.setMuted(videoMute)
    imageTrack?.setMuted?.(videoMute)
  }, [cameraTrack, screenTrack, imageTrack, videoMute])
  ...
  return (
    ...
    >
      <div className="my-3 h-60 w-full overflow-hidden rounded-lg">
        <LocalStreamPlayer videoTrack={activeTrack} />
      </div>
    </VideoDeviceWrapper>
  )
}
