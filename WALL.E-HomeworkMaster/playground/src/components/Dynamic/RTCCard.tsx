...
export default function RTCCard(props: { className?: string }) {
  ...
  const [imageTrack, setImageTrack] = React.useState<ILocalVideoTrack>()
  ...
  const onLocalTracksChanged = (tracks: IUserTracks) => {
    ...
    const { videoTrack, audioTrack, screenTrack, imageTrack } = tracks
    setVideoTrack(videoTrack)
    setScreenTrack(screenTrack)
    setImageTrack(imageTrack)
    if (audioTrack) {
  ...
  React.useEffect(() => {
    if (imageTrack && videoSourceType !== VideoSourceType.IMAGE) {
      setVideoSourceType(VideoSourceType.IMAGE)
    }
    if (!imageTrack && videoSourceType === VideoSourceType.IMAGE) {
      setVideoSourceType(VideoSourceType.CAMERA)
    }
  }, [imageTrack, videoSourceType])
      ...
      <div className="min-h-0 overflow-y-auto z-10">
        {useTrulienceAvatar ? (
          !avatarInLargeWindow ? (
            ...
            !isCompactLayout &&
            <ChatCard
              className="m-0 w-full h-full rounded-b-lg md:rounded-lg bg-transparent"
            />
          )
        ) : (
